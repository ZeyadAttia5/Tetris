...
static s8 Tetris_drawShape2(void)
{
    ...
    {
    case TETRIS_SHAPE_I:
        LOC_u8ErrCode = Tetris_draw_I(&(game_controller.board), game_controller.active_block);
        break;
    case TETRIS_SHAPE_O:
        LOC_u8ErrCode = Tetris_draw_O(&(game_controller.board), game_controller.active_block);
        break;
    case TETRIS_SHAPE_T:
        LOC_u8ErrCode = Tetris_draw_T(&(game_controller.board), game_controller.active_block);
        break;
    case TETRIS_SHAPE_J:
        LOC_u8ErrCode = Tetris_draw_J(&(game_controller.board), game_controller.active_block);
        break;
    case TETRIS_SHAPE_L:
        LOC_u8ErrCode = Tetris_draw_L(&(game_controller.board), game_controller.active_block);
        break;
    case TETRIS_SHAPE_S:
        LOC_u8ErrCode = Tetris_draw_S(&(game_controller.board), game_controller.active_block);
        break;
    case TETRIS_SHAPE_Z:
        LOC_u8ErrCode = Tetris_draw_Z(&(game_controller.board), game_controller.active_block);
        break;
...
static s8 Tetris_draw_I(DotMatrix *board, const Block new_block)
{
    ...
    case TETRIS_ROTATION_180:
        for (movements = 0; movements < MAX_PIXELS_PER_BLOCK - 1; movements++)
        {
            LOC_u8DrawStatus = board->movePixelUp(board->buffer, LOC_u8Center_x--, LOC_u8Center_y, ON_MOVE_SET_OLD);
            if (LOC_u8DrawStatus != TRUE)
            {
                break;
            }
...
s8 Tetris_rotate(void)
{
    ...
    game_controller.active_block.rotation = (game_controller.active_block.rotation + 1) % 4; // there are only 4 rotations:   0� -> 360�
