...
void EXTI_voidInit(u8 copy_u8LineID)
{
	...
	{
	case EXTI8:
		GPIO_voidSetPinMode(IOA, PIN8, INPUT);
		GPIO_voidSetPinPuPdConfig(IOA, PIN8, INPUT_PU);
		GPIO_voidSetPinSpeed(IOA, PIN8, OUTPUT_LS);
		EXTI_voidEXTILineEnable(EXTI8);
		EXTI_voidSetSenseControl(EXTI8, EXTI_SC_FallingEdge);
	...
	case EXTI9:
		...
		break;
	case EXTI10:
		GPIO_voidSetPinMode(IOA, PIN10, INPUT);
		GPIO_voidSetPinPuPdConfig(IOA, PIN10, INPUT_PU);
